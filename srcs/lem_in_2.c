...
static unsigned char	show_result(int ants, int temp[], int room[], int ant_index)
{
	...
	while (room[ant_index] != -1)	//while every ant has moved at least once.
	{
		...
		else
		{
			...
			temp[ant_index] = ((unsigned int)ant_index == get_glob()->count - 1) ? temp[ant_index] + 1 //checks if it's the last ant to move, if it is false it increases the ant index, if
...
static unsigned char	calculate_and_print(int ants, int temp[])
{
	...
	if (temp[get_glob()->count - 1] == ants)	//check if the are no more ants moving/busy.
	...
	while (move)
	{
		show_result(ants, temp, move->content, get_glob()->count - 1);	//while this function passes the if statement above. run the show_result function.
...
int				main(void)
{
	...
	temp = (int*)ft_memalloc(sizeof(int) * get_glob()->count); //allocating the variable *temp memory.
	temp[0] = get_glob()->ants;	//assigning the first index of integer array *temp, the value of the number of ants.
	if (get_glob()->ants <= 0 || ft_check_room(*get_room())) //if the number of ants are lower than 0, or if the is no start or end room.
		return (ft_error("\nERROR\n", 2)); //return error.
	if (ft_solve(get_glob()->ants, -1, -1, 0))	//runs the function ft_solve(which actually solves the map and removes unused paths)and if the function determines that the map is unsolvable.
		return (ft_error("\nERROR\n", 2)); //return error.
	ft_putchar('\n');	//print newline, to separate the return results from the map data.
	(void)calculate_and_print(get_glob()->ants, temp);	//runs calculate_and_print results which actually prints the solution of the map.
